...
def _stage_duration_seconds(stage) -> int | None:
...
def _stage_total_tokens(stage) -> int | None:
    ...
    for index, stage in enumerate(pipeline.stages):
        ...
        stages.append(
            FrontendPipelineStage(
                ...
                duration=_stage_duration_seconds(stage),
                tokens=_stage_total_tokens(stage),
...
def _build_dynamic_agents(pipelines: list[Pipeline]) -> list[FrontendAgent]:
    agent_meta = {
        StageType.REQUIREMENT: {
...
def _build_dynamic_analytics(pipelines: list[Pipeline]) -> FrontendAnalyticsOverview:
    ...
    for pipeline in pipelines:
        ...
        for stage in pipeline.stages:
            stage_tokens = _stage_total_tokens(stage) or 0
    ...
    for stage_type in StageType:
        ...
        stage_name, _ = _stage_type_label(stage_type)
...
@router.post("/pipelines/{pipeline_id}/pause", response_model=FrontendPipeline)
...
@router.post("/pipelines/{pipeline_id}/resume", response_model=FrontendPipeline)
...
@router.post("/pipelines/{pipeline_id}/cancel", response_model=FrontendPipeline)
...
@router.post("/pipelines/{pipeline_id}/retry", response_model=FrontendPipeline)
...
async def list_agents(
    ...
    persisted = await service.list_pipelines()
    if not persisted:
        return AGENTS
    return _build_dynamic_agents(persisted)
...
async def get_analytics(
    ...
    if not persisted:
        return ANALYTICS
    return _build_dynamic_analytics(persisted)
